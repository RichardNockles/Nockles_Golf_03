...
func _ready():
    print("=== GolfBall.gd _ready() START ===")
    print("🏌️ Golf Ball Ready!")
    Input.mouse_mode = Input.MOUSE_MODE_CAPTURED
    freeze = false
    print("=== GolfBall.gd _ready() END ===")
...
func _physics_process(_delta):
    if is_moving:
        print("Ball is moving.  Velocity:", linear_velocity.length()) # added to check speed
...
func _hit_ball():
    if is_aiming:
        ...
        var final_impulse = direction * shot_power * power_multiplier
        print("Final Impulse:", final_impulse)
        apply_impulse(final_impulse)
